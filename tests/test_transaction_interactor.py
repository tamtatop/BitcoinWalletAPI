...
from app.core.transaction.interactor import TransactionInteractor, MakeTransactionRequest, TransactionError, GetTransactionsRequest
from result import Err, Ok
from app.core.btc_constants import SATOSHI_IN_BTC


class TestingFeeCalculator:
    def __call__(self, source_wallet: Wallet, destination_wallet: Wallet, transfer_amount: int) -> int:
...
def test_make_transaction_one_user(
    ...
    response_err_no_user = interactor.make_transaction(MakeTransactionRequest(
        user_api_key="Blaa",
    ...
    response_ok = interactor.make_transaction(MakeTransactionRequest(
    ...
    assert response_ok.value.amount_left_btc == (init_balance - transfer_amount) / SATOSHI_IN_BTC

    response_err_no_src = interactor.make_transaction(MakeTransactionRequest(
        user_api_key=user_api_key,
        source_address="",
        destination_address=wallet_address_2,
        amount=transfer_amount,
    ))
    response_err_no_dest = interactor.make_transaction(MakeTransactionRequest(
        user_api_key=user_api_key,
        source_address=wallet_address_1,
        destination_address="",
        amount=transfer_amount,
    ...
    assert isinstance(response_err_no_src, Err) and isinstance(response_err_no_dest, Err)
    ...
    response_err_not_enough = interactor.make_transaction(MakeTransactionRequest(
        user_api_key=user_api_key,
        source_address=wallet_address_1,
        destination_address=wallet_address_2,
    ...
    assert response_err_not_enough.value == TransactionError.NOT_ENOUGH_AMOUNT_ON_SOURCE_ACCOUNT
...
def test_make_transaction_two_users(
    ...
    response_incr_api_key = interactor.make_transaction(MakeTransactionRequest(
...
def test_get_transactions(
    ...
    for i in range(100):
        user_1_wallets.append(wallet_repository.create_wallet(user_api_key_1, f"Wallet {i} of User 1", init_balance))
        user_2_wallets.append(wallet_repository.create_wallet(user_api_key_2, f"Wallet {i} of User 2", init_balance))
    ...
    for a, b in zip(user_1_wallets, user_2_wallets):
        r1 = interactor.make_transaction(MakeTransactionRequest(
            user_api_key=user_api_key_1,
        ...
        r2 = interactor.make_transaction(MakeTransactionRequest(
            user_api_key=user_api_key_2,
    ...
    response_err_no_wallet = interactor.get_transactions(GetTransactionsRequest(user_api_key_1, ""))
